...
import com.virtual.cloud.om.sdk.dto.dataReport.cas.VirtRateDTO;
...
@Slf4j
public class ClusterVirtHostCpuTopNCollector extends DataReportCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        if (CollectionUtil.isNotEmpty(clusterIds)) {
            ...
            String url = String.format(CasUriConstants.Cluster.QUERY_CLUSTER_VIRT_HOST_CPU_TOPN, clusterId);
            List<VirtRateDTO> virtRateDTOS = null;
            try {
                virtRateDTOS = this.casRestConnection.get(platform, host, protocol, port,
                        username, password, url, new ParameterizedTypeReference<List<VirtRateDTO>>() {
                        });
            } catch (Exception e) {
...
//            return virtRateDTOS;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.cluster_virt_host_cpu_TopN;
    }
