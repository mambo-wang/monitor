...
@Component
public class CasFsmcoreLogPatternHandler extends DefaultLogPatternHandler implements LogPatternApi {

    private static final String pattenExp = "([^\\[\\]]*)\\,([^\\[\\]]*)\\s([^\\[\\]]*)\\:(\\d*)\\s(\\w*)\\s(.*)";
    ...
    public Optional<LogLine> parseLine(String message) {
        ...
        if (isFind) {
            ...
            logLine.setLine(matcher.group(4));
            logLine.setMethod(matcher.group(3));
            logLine.setMessage(matcher.group(6));
    ...
    public RealTimeLogTypeEnum logType() {
        return RealTimeLogTypeEnum.cas_fsmcore;
    }
}
