...
@Component
public class CasOvsshellLogPatternHandler extends DefaultLogPatternHandler implements LogPatternApi {

    private static final String pattenExp = "([^\\[\\]]*)\\s\\[([^\\[\\]]*)\\]\\(([^\\[\\]]*)\\):\\sPID=(\\d*):\\s(\\w*):(\\w*),\\sparams\\:([^\\[\\]]*)";
    ...
    public Optional<LogLine> parseLine(String message) {
        ...
        if (isFind) {
            ...
            logLine.setScript(matcher.group(3));
            logLine.setLevel(matcher.group(5));
            logLine.setPid(matcher.group(4));
            logLine.setParams(matcher.group(7));
    ...
    public RealTimeLogTypeEnum logType() {
        return RealTimeLogTypeEnum.cas_ovsshell;
    }
}
