...
@Component
public class CasNactlLogPatternHandler extends DefaultLogPatternHandler implements LogPatternApi {

    private static final String pattenExp = "([^\\[\\]]*)\\s([^\\[\\]]*)\\s(\\w*)\\s(\\w*):(\\d*)(.*)";
    ...
    public Optional<LogLine> parseLine(String message) {
        ...
        if (isFind) {
            ...
            logLine.setLine(matcher.group(5));
            logLine.setMethod(matcher.group(4));
    ...
    public RealTimeLogTypeEnum logType() {
        return RealTimeLogTypeEnum.cas_nactl;
    }
}
