...
import com.virtual.cloud.om.sdk.constant.OperationLogTypeEnum;
...
@Slf4j
public class CasVmTaskLogReportCollector extends DataReportCollector {
    ...
    public List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        Integer limit = 2000;
        Integer offset = 0;
        String uri = String.format(CasUriConstants.Log.OPERATION_LOGS_VM, OperationLogTypeEnum.vm.type, limit, offset);
        RpcResult<List<VmTaskLogDto>> listRpcResult;
        try {
            listRpcResult = casTokenRestConnection.get(host, protocol, username, password, port, uri, new ParameterizedTypeReference<RpcResult<List<VmTaskLogDto>>>() {
            }).getBody();
            Utils.checkResult(uri, listRpcResult);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.cas_operation_log;
    }
