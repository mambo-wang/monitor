...
@Slf4j
public class ConnnectDomainCollector extends DataReportCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = domainIds.parallelStream().map(s -> {
            String url = String.format(CasUriConstants.Domain.QUERY_DOMAIN_LINK_NUM, s);
            ...
            try {
                ...
                log.error("cas connection is fail : " + e);
            }
            ...
            try {
                int rateInt = Double.valueOf(rate.getRate()).intValue();
                dataValueAndTagsDTO.setValue(rateInt);
            } catch (NumberFormatException e) {
                log.error("cas [connection] fail resourceId:{},domainId:{} ", resourceId, s);
            }
        ...
        log.debug("[connection]==================>>采集完成：size=" + finalResult.size());
        return finalResult;
    }

    @Override
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.connection;
    }
