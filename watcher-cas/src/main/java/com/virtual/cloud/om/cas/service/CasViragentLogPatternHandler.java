...
@Component
public class CasViragentLogPatternHandler extends DefaultLogPatternHandler implements LogPatternApi {

    private static final String pattenExp = "([^\\[\\]]*)\\+([^\\[\\]]*)\\:\\s([\\d]*):\\s([^\\[\\]]*)\\s:\\s([^\\[\\]]*)\\:([^\\[\\]]*)\\s\\:\\s(.*)";
    ...
    public Optional<LogLine> parseLine(String message) {
        ...
        if (isFind) {
            ...
            logLine.setLine(matcher.group(6));
            logLine.setMethod(matcher.group(5));
    ...
    public RealTimeLogTypeEnum logType() {
        return RealTimeLogTypeEnum.cas_viragent;
    }
}
