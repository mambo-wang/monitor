...
import com.virtual.cloud.om.sdk.dto.dataReport.cas.ParameterDTO;
...
@Slf4j
public class HostSummaryCollector extends DataReportCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        if (CollectionUtil.isNotEmpty(hostIds)) {
            ...
            String url = String.format(CasUriConstants.Host.QUERY_HOST_SUMMARY, hostId);
            List<ParameterDTO> hostSummarys = null;
            try {
                //todo
                hostSummarys = this.casRestConnection.get(platform, host, protocol, port,
                        username, password, url, new ParameterizedTypeReference<List<ParameterDTO>>() {
                        });
            } catch (Exception e) {
...
//            return hostSummarys;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.host_summary;
    }
