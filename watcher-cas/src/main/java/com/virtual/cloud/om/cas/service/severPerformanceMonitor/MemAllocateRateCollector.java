...
@Slf4j
public class MemAllocateRateCollector extends DataReportCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        if (CollectionUtil.isEmpty(hostIds)) {
            List<String> hostIdList = findAllIds.getHostIds(platform, host, protocol, port, username, password, tags, resourceId);
        ...
        List<DataValueAndTagsDTO> finalResult = hostIds.parallelStream().map(s -> {
            ...
            try {
                ...
                log.error("cas mem_allocate_rate is fail : " + e);
            }
            ...
            Long time = null;
            try {
                dataValueAndTagsDTO.setValue(hostDetailInfoDTOS.getMemorySuperRatio());
        ...
        log.debug("[mem_allocate_rate]==================>>采集完成：size=" + finalResult.size());
        return finalResult;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.mem_allocate_rate;
    }
