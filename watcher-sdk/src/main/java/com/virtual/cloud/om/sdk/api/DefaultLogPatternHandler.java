 ...
 */
@Component
public abstract class DefaultLogPatternHandler implements LogPatternApi {

    public static final String pattenExp = "([^\\[\\]]*)\\[([^\\[\\]]*)\\]\\[([^\\[\\]]*)\\]\\[([^\\[\\]]*)\\]\\[([^\\[\\]]*)\\]([^\\[\\]]*)\\s+\\[([^\\[\\]]*)\\]\\s+-\\s+([^\\[\\]]*)(.*)";
    private Pattern pattern = Pattern.compile(pattenExp);

    @Override
    public synchronized Optional<LogLine> parseLine(String message) {
        ...
        if (isFind) {
            ...
            logLine.setLine(matcher.group(9));
            logLine.setMessage(matcher.group(10));
    ...
    public RealTimeLogTypeEnum logType() {
        return RealTimeLogTypeEnum.default_;
    }
}
