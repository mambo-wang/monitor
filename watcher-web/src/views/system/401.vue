<template>
  <div class="wscn-http404-container layout-container flex center">
    <div class="wscn-http404">
      ...
      <div class="bullshit">
        <div class="bullshit__oops">  {{ $t("message.system.noPermission") }}</div>
        <div class="bullshit__info">
          {{ $t("message.system.error401Tips") }}
          
        </div>
        <el-button @click="go" type="primary" round>{{ $t("message.system.goHome") }}</el-button>
      </div>
    </div>
  </div>
</template>
...
const go = () => {
  if (
    ...
    let permissionsList =
      JSON.parse(localStorage.getItem("vuex") || "").user.info.permissions ||
      [];
      let homePage="" 
    permissionsList.forEach((item: any) => {
      if (item === "FLOW_TEMPLATE") {
homePage="/"
      }
    });
    if(homePage==="/"){
