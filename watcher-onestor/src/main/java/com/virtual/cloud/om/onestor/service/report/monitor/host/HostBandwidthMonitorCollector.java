...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.host.OneStorHostBandwidthMonitorReportDTO;
...
/**
 * OneStor 主机 bandwidth监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class HostBandwidthMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorHostMonitorTargetEnum.storage_read_bw.getValue(), hostName));
            url.append(appendTarget(OneStorHostMonitorTargetEnum.storage_write_bw.getValue(), hostName));
            url.append(appendTarget(OneStorHostMonitorTargetEnum.fs_read_bw.getValue(), hostName));
            url.append(appendTarget(OneStorHostMonitorTargetEnum.fs_write_bw.getValue(), hostName));
            ...
            OneStorHostBandwidthMonitorReportDTO reportDTO = new OneStorHostBandwidthMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setHostName(hostName)
                    .setStorageReadBw(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.storage_read_bw.getValue(), hostName))
                    .setStorageWriteBw(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.storage_write_bw.getValue(), hostName))
                    .setFsReadBw(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.fs_read_bw.getValue(), hostName))
                    .setFsWriteBw(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.fs_write_bw.getValue(), hostName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.host_bandwidth;
    }
