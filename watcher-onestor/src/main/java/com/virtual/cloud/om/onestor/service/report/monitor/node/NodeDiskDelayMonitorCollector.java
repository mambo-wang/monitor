...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.node.OneStorDiskDelayMonitorReportDTO;
...
/**
 * OneStor 节点池 DiskDelay监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class NodeDiskDelayMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = nodePoolDTOList.parallelStream().map(nodePool -> {
            ...
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.lat_read.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.lat_wite.getValue(), nodePoolName));
            ...
            OneStorDiskDelayMonitorReportDTO reportDTO = new OneStorDiskDelayMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setPoolName(nodePoolName)
                    .setLatRead(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.lat_read.getValue(), nodePoolName))
                    .setLatWrite(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.lat_wite.getValue(), nodePoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.node_disk_delay;
    }
