...
@Slf4j
public class StorageIOPSMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            Long id = basicDTO.getId();
            StringBuffer url = new StringBuffer(OnestoreUriConstants.Monitor.MONITOR_BY_TARGET);
            url.append(appendTarget(OneStorStorageMonitorTargetEnum.iops_read.getValue(), noodPoolName, id.toString()));
            url.append(appendTarget(OneStorStorageMonitorTargetEnum.iops_write.getValue(), noodPoolName, id.toString()));
            url.append(appendTarget(OneStorStorageMonitorTargetEnum.onestor_pool_bw_all.getValue(), noodPoolName, id.toString()));
            url.append(appendTarget(OneStorStorageMonitorTargetEnum.onestor_pool_iops_all.getValue(), noodPoolName, id.toString()));
            ...
            reportDTO.setNodePoolName(poolName)
                    .setAllIops(getData(oneStorMonitorDTOMap,OneStorStorageMonitorTargetEnum.onestor_pool_iops_all.getValue(),noodPoolName,id.toString()))
                    .setAllBw(getData(oneStorMonitorDTOMap,OneStorStorageMonitorTargetEnum.onestor_pool_bw_all.getValue(),noodPoolName,id.toString()))
                    .setIopsRead(getData(oneStorMonitorDTOMap, OneStorStorageMonitorTargetEnum.iops_read.getValue(), noodPoolName,id.toString()))
                    .setIopsWrite(getData(oneStorMonitorDTOMap, OneStorStorageMonitorTargetEnum.iops_write.getValue(), noodPoolName,id.toString()))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.storage_iops;
    }
