...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.node.OneStorNodeMemDTO;
...
@Slf4j
public class NodeMemUsageMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = nodePoolDTOList.parallelStream().map(nodePool -> {
            ...
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.sub_cluster_node_mem_ratio.getValue(), nodePoolName));
            ...
            OneStorNodeMemDTO reportDTO = new OneStorNodeMemDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setPoolName(nodePoolName)
                    .setMemRatio(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.sub_cluster_node_mem_ratio.getValue(), nodePoolName));
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.node_mem_usage;
    }
