...
@Slf4j
public class StorClusterMemUsageCollector extends DataReportCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        String url = OnestoreUriConstants.Cluster.STOR_CLUSTER_MONITOR
                + Constant.TARGET + OneStorClusterMonitorEnum.big_cluster_mem_ratio.getValue();
        ...
        List<List> mem_ratio_list = stringListMap.get(OneStorClusterMonitorEnum.big_cluster_mem_ratio.getValue());


        Double mem_ratio_value=(Double)mem_ratio_list.get(mem_ratio_list.size() - 1).get(0);
        Integer mem_ratio_time=(Integer)mem_ratio_list.get(mem_ratio_list.size() - 1).get(1);
        Long time = Long.valueOf(mem_ratio_time) * 1000;
        String formatFullDateTime = Utils.formatFullDateTime(time);
        StorClusterMemUsageDTO dto=new StorClusterMemUsageDTO();
        dto.setClusterName(clusterId.getName());
        dto.setMemRatio(mem_ratio_value);
        ...
        log.info("stor_cluster_mem_usage : {}",Lists.newArrayList(dataValueAndTagsDTO));
        return Lists.newArrayList(dataValueAndTagsDTO);
    }

    @Override
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.stor_cluster_mem_usage;
    }
