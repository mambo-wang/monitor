...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.host.OneStorHostMemUsageMonitorReportDTO;
...
/**
 * OneStor 主机 disk_delay监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class HostDiskdelayCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorHostMonitorTargetEnum.lat_read.getValue(), hostName));
            url.append(appendTarget(OneStorHostMonitorTargetEnum.lat_wite.getValue(), hostName));
            ...
            OneStorHostDiskDelayMonitorReportDTO reportDTO = new OneStorHostDiskDelayMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setHostName(hostName)
                    .setLatRead(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.lat_read.getValue(), hostName))
                    .setLatWrite(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.lat_wite.getValue(), hostName))
            ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.host_disk_delay;
    }
