...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.host.OneStorHostCapacityMonitorReportDTO;
...
/**
 * OneStor 主机 capacity监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class HostCapacityMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorHostMonitorTargetEnum.capacity_total.getValue(), hostName));
            url.append(appendTarget(OneStorHostMonitorTargetEnum.capacity_used.getValue(), hostName));
            ...
            OneStorHostCapacityMonitorReportDTO reportDTO = new OneStorHostCapacityMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setHostName(hostName)
                    .setTotal(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.capacity_total.getValue(), hostName))
                    .setUsed(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.capacity_used.getValue(), hostName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.host_capacity;
    }
