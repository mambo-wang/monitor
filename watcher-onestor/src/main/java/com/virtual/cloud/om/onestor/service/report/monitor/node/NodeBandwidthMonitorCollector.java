...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.node.OneStorNodeBandwidthMonitorReportDTO;
...
/**
 * OneStor 节点池 Bandwidth监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class NodeBandwidthMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = nodePoolDTOList.parallelStream().map(nodePool -> {
            ...
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.storage_read_bw.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.storage_write_bw.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.storage_recover_bw.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.fs_read_bw.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.fs_write_bw.getValue(), nodePoolName));
            url.append(Constant.TARGET+ OneStorClusterMonitorEnum.sub_cluster_node_pool_all_bw_all.getValue());
            url.append(Constant.TARGET+ OneStorClusterMonitorEnum.sub_cluster_node_fs_bw_total.getValue());
            ...
            OneStorNodeBandwidthMonitorReportDTO reportDTO = new OneStorNodeBandwidthMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setPoolName(nodePoolName)
                    .setAllBw(getData(oneStorMonitorDTOMap,OneStorClusterMonitorEnum.sub_cluster_node_pool_all_bw_all.getValue(),""))
                    .setFsTotalBw(getData(oneStorMonitorDTOMap,OneStorClusterMonitorEnum.sub_cluster_node_fs_bw_total.getValue(),""))
                    .setStorageReadBw(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.storage_read_bw.getValue(), nodePoolName))
                    .setStorageWriteBw(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.storage_write_bw.getValue(), nodePoolName))
                    .setStorageRecoverBw(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.storage_recover_bw.getValue(), nodePoolName))
                    .setFsReadBw(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.fs_read_bw.getValue(), nodePoolName))
                    .setFsWriteBw(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.fs_write_bw.getValue(), nodePoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.node_bandwidth;
    }
