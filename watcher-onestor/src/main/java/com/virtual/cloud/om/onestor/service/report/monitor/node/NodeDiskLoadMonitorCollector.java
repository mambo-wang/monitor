...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.node.OneStorNodeDiskLoadMonitorReportDTO;
...
/**
 * OneStor 节点池 DiskLoad监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class NodeDiskLoadMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = nodePoolDTOList.parallelStream().map(nodePool -> {
            ...
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.util_avg.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.util_max.getValue(), nodePoolName));
            ...
            OneStorNodeDiskLoadMonitorReportDTO reportDTO = new OneStorNodeDiskLoadMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setPoolName(nodePoolName)
                    .setUtilAvg(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.util_avg.getValue(), nodePoolName))
                    .setUtilMax(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.util_max.getValue(), nodePoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.node_disk_load;
    }
