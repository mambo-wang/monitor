...
/**
 * OneStor 主机 mem_usage监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class HostMemUsageMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorHostMonitorTargetEnum.mem_usage.getValue(), hostName));
            ...
            OneStorHostMemUsageMonitorReportDTO reportDTO = new OneStorHostMemUsageMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setHostName(hostName)
                    .setMemRatio(getData(oneStorMonitorDTOMap, OneStorHostMonitorTargetEnum.mem_usage.getValue(), hostName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.stor_host_mem_usage;
    }
