...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.disk.OneStorDiskPoolBandwidthMonitorReportDTO;
...
/**
 * OneStor 硬盘池 bandwidth监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class DiskPoolBandwidthMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorDiskPoolMonitorTargetEnum.storage_read_bw.getValue(), diskPoolName));
            url.append(appendTarget(OneStorDiskPoolMonitorTargetEnum.storage_write_bw.getValue(), diskPoolName));
            url.append(appendTarget(OneStorDiskPoolMonitorTargetEnum.storage_recover_bw.getValue(), diskPoolName));
            ...
            OneStorDiskPoolBandwidthMonitorReportDTO reportDTO = new OneStorDiskPoolBandwidthMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setDiskpoolName(diskPoolName)
                    .setStorageReadBw(getData(oneStorMonitorDTOMap, OneStorDiskPoolMonitorTargetEnum.storage_read_bw.getValue(), diskPoolName))
                    .setStorageWriteBw(getData(oneStorMonitorDTOMap, OneStorDiskPoolMonitorTargetEnum.storage_write_bw.getValue(), diskPoolName))
                    .setStorageRecoverBw(getData(oneStorMonitorDTOMap, OneStorDiskPoolMonitorTargetEnum.storage_recover_bw.getValue(), diskPoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.diskpool_bandwidth;
    }
