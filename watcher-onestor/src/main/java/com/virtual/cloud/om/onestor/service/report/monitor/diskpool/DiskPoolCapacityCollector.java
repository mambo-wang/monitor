...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.disk.OneStorDiskPoolCapacityMonitorReportDTO;
...
/**
 * OneStor 硬盘池 capacity监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class DiskPoolCapacityCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = basicDTOList.parallelStream().map(basicDTO -> {
            ...
            url.append(appendTarget(OneStorDiskPoolMonitorTargetEnum.capacity_total.getValue(), diskPoolName));
            url.append(appendTarget(OneStorDiskPoolMonitorTargetEnum.capacity_used.getValue(), diskPoolName));
            ...
            OneStorDiskPoolCapacityMonitorReportDTO reportDTO = new OneStorDiskPoolCapacityMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setDiskpoolName(diskPoolName)
                    .setTotal(getData(oneStorMonitorDTOMap, OneStorDiskPoolMonitorTargetEnum.capacity_total.getValue(), diskPoolName))
                    .setUsed(getData(oneStorMonitorDTOMap, OneStorDiskPoolMonitorTargetEnum.capacity_used.getValue(), diskPoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.diskpool_capacity;
    }
