...
import com.virtual.cloud.om.sdk.dto.dataReport.onestor.monitor.node.OneStorNodeCapacityMonitorReportDTO;
...
/**
 * OneStor 节点池 CpuUsage监控信息采集器
 * */
@Service
@RequiredArgsConstructor
@Slf4j
public class NodeCpuUsageMonitorCollector extends DataReportCollector implements IOnestorMonitorCollector {
    ...
    protected List<DataValueAndTagsDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        List<DataValueAndTagsDTO> finalResult = nodePoolDTOList.parallelStream().map(nodePool -> {
            ...
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.capacity_total.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.capacity_used.getValue(), nodePoolName));
            url.append(appendTarget(OneStorNodePoolMonitorTargetEnum.sub_cluster_node_cpu_ratio.getValue(), nodePoolName));
            ...
            OneStorNodeCapacityMonitorReportDTO reportDTO = new OneStorNodeCapacityMonitorReportDTO();
            Date currentDate = getTime(oneStorMonitorDTOMap);
            reportDTO.setPoolName(nodePoolName)
                    .setCpuRatio(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.sub_cluster_node_cpu_ratio.getValue(), nodePoolName))
                    .setTotal(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.capacity_total.getValue(), nodePoolName))
                    .setUsed(getData(oneStorMonitorDTOMap, OneStorNodePoolMonitorTargetEnum.capacity_used.getValue(), nodePoolName))
                    ;
    ...
    public DataReportTypeByMetricEnum metric() {
        return DataReportTypeByMetricEnum.node_cpu_usage;
    }
