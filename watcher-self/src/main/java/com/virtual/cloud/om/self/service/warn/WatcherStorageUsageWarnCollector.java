...
@Slf4j
public class WatcherStorageUsageWarnCollector extends WarnReportCollector {
    ...
    protected List<WarnDataDTO> collect(String platform, String host, String protocol, Integer port, String username, String password, String tags, String resourceId) {
        ...
        deploys.stream().forEach(deploy -> {
            ...
            try {
                ...
                log.info("节点IP为:{},根路径磁盘利用率处理前为:{}", ip, storageUsage);
                long storageUsageLong = Long.parseLong(storageUsage);
                if (storageUsageLong >= WarnConstant.WatcherWarn.WARN_BASIC_COUNT && storageUsageLong < WarnConstant.WatcherWarn.WARN_URGENT_COUNT) {
                    dto.setType(WarnConstant.WatcherWarn.WATCHER_WARN_TYPE);
                    dto.setSrc("能力中心Agent（"+ip+"）");
                    dto.setName(WarnConstant.eventName.STORAGE_USAGE_NAME);
                    dto.setObjectType(WarnConstant.ObjectType.WATCHER_OBJECTTYPE);
                    dto.setMessage("能力中心Agent根路径磁盘利用率达到或超过90%");
                    ...
                    dto.setWatcherIp(ip);
                    value.add(dto);
                } else if (storageUsageLong == WarnConstant.WatcherWarn.WARN_URGENT_COUNT){
                    dto.setType(WarnConstant.WatcherWarn.WATCHER_WARN_TYPE);
                    dto.setSrc("能力中心Agent（"+ip+"）");
                    dto.setName(WarnConstant.eventName.STORAGE_USAGE_NAME);
                    dto.setObjectType(WarnConstant.ObjectType.WATCHER_OBJECTTYPE);
                    dto.setMessage("能力中心Agent根路径磁盘利用率为100%");
                    dto.setStartsAt(System.currentTimeMillis());
                    dto.setEndsAt(System.currentTimeMillis());
        ...
        log.info("[watcher storage usage value={}]", value);
        return value;
    }

    @Override
    public WarnMetricEnum metric() {
        return WarnMetricEnum.watcher_storage_warn;
    }
